    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-f1-dark/95 backdrop-blur border-b border-f1-border">
      <div className="flex items-center justify-between px-4 h-14 max-w-screen-xl mx-auto">
        <Link to="/" className="flex items-center gap-2">
          <span className="text-f1-red font-black text-xl tracking-tighter leading-none">GF1</span>
          <span className="text-white/40 text-xs font-medium tracking-widest uppercase hidden xs:block">Formula 1</span>
        </Link>

        {/* Desktop nav */}
        <nav className="hidden md:flex items-center gap-1">
          {NAV_LINKS.map(link => {
            const active = pathname === link.to || (link.to !== '/' && pathname.startsWith(link.to.split('/')[1] !== '' ? `/${link.to.split('/')[1]}` : '/'));
            return (
              <Link
                key={link.to}
                to={link.to}
